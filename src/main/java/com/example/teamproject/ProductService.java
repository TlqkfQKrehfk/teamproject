...
//		createProduct(CreateRequest request) {
//			// 요청 객체를 새로운 엔티티 객체로 변환
//			Product newOrder = Product.builder().address(request.getAddress()).amount(request.getAmount())
//					.name(request.getName()).created_by(request.getName()).status("00").build();
//			// 부모 레코드 저장
//			orderRepo.save(newOrder); // repo.save(entity)->entity객체에 id갑이 생성이 됨
//			// 데이터베이스에서 실제로 저장된 객체를 다시 조회하여 응답으로 돌려줄거임
//			Product savedOrder = orderRepo.getOne(newOrder.getId());
//
//			// 자식 엔티티 객체 목록 초기화
//			savedOrder.setProductFile(new ArrayList<ProductDetail>());
//			// 요청받은 자식 객체 목록을 엔티티 객체로 변환 및 저장
//			for (CreateRequest.Detail detail : request.getFile()) {
//				ProductDetail newDetail = ProductDetail.builder().ProductId(savedOrder.getId())
